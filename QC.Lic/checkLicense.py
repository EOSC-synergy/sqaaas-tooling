...
def is_license_present_github(url):
    ...
    try:
        ...
        print("Found license for repository %s: %s" % (
            g_repo.name, repo_license.license
        ))
        return True
...
def is_file_in_gitlab_repo(server, repo, file):
    ...
    try:
        ...
        for item in items:
            if item['name'] == file:
                print("File %s found in GitLab repository %s" % (
                    file, repo
                ))
                return True
        ...
        return False
    print("File %s not found in GitLab repository %s" % (
        file, repo
    ))
    return False
